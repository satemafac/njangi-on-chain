...
| Duplicate round open (2026-08-30 incident) | **UI half shipped; Move half code complete, publish-gated (2026-09-06).** Two escrows were minted for one round 34s apart because the panel's open controls cleared their busy flag on tx resolution and a stale re-read re-offered "open". UI: one in-flight lock across every open control, held until discovery confirms the new escrow or a 90s timeout (`src/lib/cycle-open-round-lock.ts`, unit-tested); a `refunded` stage replaces the misleading in-progress render. Move (v1.2, additive only): `FIELD_OPEN_ROUND` marker written by the indexed opens, `E_ROUND_ALREADY_OPEN` (234) in `open_cycle_internal`, cleared by `advance_circle_after_claim`, and permissionless `release_open_round` for refunded/abandoned escrows (`E_ROUND_STILL_OPEN`, 235). 183/183 Move tests. Client chains the release behind `NEXT_PUBLIC_ESCROW_ROUND_GUARD_ENABLED` (default OFF) — **flip only after that package is published**; the guard itself needs no flag. |
